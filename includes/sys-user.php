...
class SysUser{
	 ...
	 * @var array
	 */
	private $fieldsMeta = array('id','user_id','meta_name','meta_value');
	...
	public function login($ip,$login,$passwd,$forever=false){
		if($this->logged($ip) == true){
			...
			$attrs = array(':login'=>array($login,PDO::PARAM_STR | PDO::PARAM_INPUT_OUTPUT),':passwd'=>array($passwdSha1,PDO::PARAM_STR));
	 ...
	 * 判断登录状态
	 * <p>如果超时，则自动退出</p>
	 * @param string $ip IP地址，判定永久在线时需要这个参数
		...
		if($nowID > 0){
			...
			if($res){
				if($res['user_status'] == $this->status['forever'] && $res['user_ip'] == $ip){
					//如果永久在线
					return $res['id'];
				}else if($res['user_status'] == $this->status['logged']){
					//否则普通登录，则判定登录时间限制
				...
				if($countRes < $countPowers){
					foreach($powers as $k=>$v){
						if(isset($res[$k]) == true){
							...
							if(!$this->db->sqlUpdate($this->tableNameMeta,$sets,$where,$attrs)) return false;
						}else{
							if(!$this->addMeta($userID,$this->powerMetaName,$v)) return false;
						}
					}
					return true;
					...
					foreach($res as $k=>$v){
						if(isset($powers[$k]) == true){
							...
							if(!$this->delMeta($v['id'])) return false;
						}
					}
					return true;
				}else{
	 ...
	 */
	private function getPasswd($passwd){
		return sha1($passwd);
	}
	...
	private function clearLoginStatus(){
		if(isset($_SESSION[$this->sessionLoginName]) == true){
			...
			unset($_SESSION[$this->sessionLoginName]);
		}
	}
